...
class ToonPayBot:
    ...
    def setup_handlers(self):
        ...
        self.application.add_handler(CommandHandler("start", UserHandlers.start, filters=filters.ChatType.PRIVATE))
        
        # Admin group commands
        self.application.add_handler(CommandHandler("tickets", AdminHandlers.tickets))
        self.application.add_handler(CommandHandler("data", AdminHandlers.data))
        self.application.add_handler(CommandHandler("getdata", AdminHandlers.getdata))
        self.application.add_handler(CommandHandler("reply", AdminHandlers.reply_command))
        
        # Super admin commands (private only)
        self.application.add_handler(CommandHandler("panel", SuperAdminHandlers.panel, filters=filters.ChatType.PRIVATE))
        
        # Group commands
        self.application.add_handler(CommandHandler("support", GroupHandlers.support))
        ...
        self.application.add_handler(CallbackQueryHandler(
            ...
            pattern='^(new_ticket|my_tickets|help|main_menu|cat_|view_ticket_|cancel)$'
        ))
        
        # Admin callbacks
        self.application.add_handler(CallbackQueryHandler(
            AdminHandlers.handle_callback,
            pattern='^(inprogress_|close_|reply_|viewuser_|filter_|back_to_ticket_)'
        ))
        
        # Super admin callbacks
        self.application.add_handler(CallbackQueryHandler(
            SuperAdminHandlers.handle_callback,
            pattern='^sa_'
        ))
