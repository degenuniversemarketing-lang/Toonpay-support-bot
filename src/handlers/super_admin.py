...
async def add_group(message: Message):
    ...
    try:
        group_id = int(message.text.replace("/addgroup", "").strip())
...
async def remove_group(message: Message):
    ...
    try:
        group_id = int(message.text.replace("/removegroup", "").strip())
...
async def get_all_data(message: Message):
    ...
    async for session in get_db():
        ...
        with open('toonpay_export.xlsx', 'wb') as f:
            f.write(excel_file.getvalue())
        
        await message.reply_document(
            FSInputFile('toonpay_export.xlsx'),
            caption="✅ Complete data export"
        )
        
        # Clean up
        import os
        os.remove('toonpay_export.xlsx')
...
async def edit_command(message: Message):
    ...
    try:
        parts = message.text.replace("/editcmd", "").strip().split("|")
        if len(parts) != 2:
